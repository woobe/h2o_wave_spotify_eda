...
async def serve(q: Q):
    ...
    if not q.client.initialized:
        # Users visiting the app for the first time will see a demo dataset and have the option to upload their own
        q.client.data_path = "./data"

        if not os.path.exists(q.client.data_path):
            os.mkdir(q.client.data_path)

        q.client.working_file_path = os.path.join(
            q.client.data_path, "example_streaming_history.json"
        )
    ...
    build_dashboards(q)
    q.page["Stream_History"] = ui.form_card(
        box="1 1 3 6",
        items=[
            ui.text_xl(
                "Upload your Spotify Stream History (.json)",
                tooltip="It should be in Json format",
            ),
            ui.file_upload(name="datasets", label="Upload", multiple=True),
            ui.text_l("Example dataset - https://bit.ly/3JhoOOa"),
        ],
    )
    await q.page.save()
...
async def handle_uploaded_data(q: Q):
    ...
    q.client.working_file_path = await q.site.download(
        url=q.args.datasets[0], path=data_path
    )


def build_dashboards(q: Q):
    ...
    df["Month"] = pd.to_datetime(df["endTime"], format="%Y-%m-%d %H:%M").dt.strftime(
        "%Y-%m"
    )
    df["DayOrNot"] = np.where(
        pd.to_datetime(df["endTime"], format="%Y-%m-%d %H:%M").dt.dayofweek < 5, 0, 1
    )
    ...
    q.page.add(
        ...
        ui.plot_card(
            ...
            data=data(
                fields=df_hour.columns.tolist(), rows=df_hour.values.tolist(), pack=True
            ),
            plot=ui.plot(
                [
                    ui.mark(
    ...
    q.page.add(
        ...
        ui.plot_card(
            ...
            data=data(
                fields=df_Day.columns.tolist(), rows=df_Day.values.tolist(), pack=True
            ),
            plot=ui.plot(
                [
                    ui.mark(
                        type="interval", x="=Day", y="=Minutes", y_min=0, color="blue"
                    )
                ]
            ),
        ),
    ...
    q.page.add(
        ...
        ui.plot_card(
            ...
            data=data(
                fields=df_month.columns.tolist(),
                rows=df_month.values.tolist(),
                pack=True,
            ),
            plot=ui.plot(
                [
                    ui.mark(
                        type="interval", x="=Month", y="=Minutes", y_min=0, color="blue"
                    )
                ]
            ),
        ),
    ...
    df_monthly_artists = (
        df.groupby(["Month", "artistName"])
        .agg({"Minutes": "sum"})
        .reset_index()
        .sort_values(["Month", "artistName"], ascending=False)
    )
    df_monthly_artists["Rank"] = df_monthly_artists.groupby("Month").rank(
        ascending=False, method="first"
    )["Minutes"]
    df_monthly_artists = df_monthly_artists.loc[
        df_monthly_artists.Rank == 1
    ...
    q.page.add(
        ...
        ui.plot_card(
            ...
            data=data(
                fields=df_monthly_artists.columns.tolist(),
                rows=df_monthly_artists.values.tolist(),
                pack=True,
            ),
            plot=ui.plot(
                [
    ...
    df_artist1 = (
        df.groupby(["Month", "artistName"]).agg({"Minutes": "sum"}).reset_index()
    )
    ...
    months_and_artists["key"] = (
        months_and_artists["Month"] + months_and_artists["artistName"]
    )
    months_and_artists = pd.merge(
        months_and_artists, df_artist2[["key", "Minutes"]], how="left", on="key"
    )
    months_and_artists.drop("key", 1, inplace=True)
    ...
    q.page.add(
        ...
        ui.plot_card(
            ...
            data=data(
                fields=months_and_artists.columns.tolist(),
                rows=months_and_artists.values.tolist(),
                pack=True,
            ),
            plot=ui.plot(
                [
    ...
    first_date = str(
        datetime.strptime(str(df.endTime.min()), "%Y-%m-%d %H:%M:%S")
        .strftime("%d-%b-%Y")
        .upper()
    )
    avg_session_duration = str(
        round(df.groupby("Session_Id").agg({"Minutes": "sum"})["Minutes"].mean())
    )
    q.page.add(
        "info_card",
        ui.form_card(
    ...
    q.page.add(
        ...
        ui.form_card(
            ...
            items=[
                ui.text(
                    "Your longest session has a total duration of "
                    + str(longest_session_time)
                    + " minutes.",
                    size="l",
                    width="200px",
                ),
                ui.text(
                    "In this session, you listened:  " + listToStr,
                    size="l",
                    width="200px",
                ),
            ],
            ...
            items=[
                ui.table(
                    ...
                    columns=[
                        ...
                        for x in df_top_artists.columns.values
                    ],
                    rows=[
                        ui.table_row(
                            name=str(i),
                            cells=[
                                str(df_top_artists[col].values[i])
                                for col in df_top_artists.columns.values
                            ],
                        )
    ...
    df_top_songs = (
        df.groupby(["trackName", "artistName"]).agg({"Minutes": "count"}).reset_index()
    )
    df_top_songs = df_top_songs.rename(
        columns={
            "trackName": "trackName",
        ...
        ui.form_card(
            ...
            items=[
                ui.table(
                    ...
                    columns=[
                        ui.table_column(name=str(x), label=str(x), sortable=True)
                        for x in df_top_songs.columns.values
                    ],
                    rows=[
                        ui.table_row(
                            name=str(i),
                            cells=[
                                str(df_top_songs[col].values[i])
                                for col in df_top_songs.columns.values
                            ],
                        )
                        for i in range(n_rows)
                    ],
